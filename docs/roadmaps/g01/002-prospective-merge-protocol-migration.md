# g01.002 — Prospective-merge protocol migration
...
northstar-language-packs' Queue control manifest uses `paseo.queue.control.v4`
and evaluates the required pre-merge hook against the exact prospective merge
candidate instead of the reviewed head.
...
- **State:** ready; configuration maintenance independent of package work.
- **Owned mutable paths:** `.paseo/queue.json` only.
- **Worker:** automatic mechanical/general pool with independent review.
- **Excluded:** language packages, registry promotion, releases, CI, product
...
successor; the registry promotion and Convergence canary stay Northstar-owned.
